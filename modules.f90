    ...
    type CAMBparams
        ...
        real(dl)  :: omegab, omegac, omegav, omegan, omegagdm, gdmfrac  !modified here by AD
        !Omega baryon, CDM, Lambda and massive neutrino
        real(dl)  :: H0,TCMB,yhe,Num_Nu_massless,zf,af,wf         !modified here by AD
        real(dl)  :: alpha0  !gluon coupling strength at a_f
        real(dl)  :: Tf_gluon  !gluon temperature at a_f
        logical   :: gluon_tca, cont_w  !gluon tight coupling approximation and continuous EoS
        integer   :: N_dark  !N of dark su(N)
        real(dl)  :: fixedqh, fixedq_final_redshift  !if fixedq /= 0, transfer functions are calculated for that k mode until fixedq_final_redshift
        character(len=40) :: evolve_metric_file, evolve_perturb_file
    ...
    real(dl) grhom,grhog,grhor,grhob,grhoc,grhov,grhornomass,grhok,grhogdm  !modified here by AD
    ...
    grhogdm=grhom*CP%omegagdm  !modified here by AD, no af check here since this subroutine reprints the parameters at tau_0 today
    !  adotrad gives the relation a(tau) in the radiation era:
    adotrad = sqrt((grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates))+grhogdm*CP%af)/3)   !modified here by AD
    ...
    if (present(error)) then
        ...
        write(*,'("Om_gdm h^2           = ",f9.6)') CP%omegagdm*(CP%H0/100)**2  !modified here by AD
    ...
    omdmh2 = (CP%omegac+CP%omegan+CP%omegagdm)*(CP%h0/100.0d0)**2  !modified here by AD, af condition is not added since omegagdm is anyway small before
...
500     if (L< lmax_extrap_highl) &
    ...
    integer, parameter :: Transfer_kh =1, Transfer_cdm=2,Transfer_b=3,Transfer_g=4, &
        ...
        Transfer_gdm=7,  & !modified here by AD
    ...
    character(LEN=name_tag_len) :: Transfer_name_tags(Transfer_max-1) = &
        ['CDM     ', 'baryon  ', 'photon  ', 'nu      ', 'mass_nu ','gdm     ', 'total   ', &
    ...
    call Recombination_Init(CP%Recomb, CP%omegagdm+CP%omegac, CP%omegab,CP%Omegan, CP%Omegav, &  !modified here by AD, omegagdm+omegac is passed instead of omegac
    ...
    a_verydom = AccuracyBoost*5*(grhog+grhornomass)/(grhoc+grhob+grhogdm)  !modified here by AD
    ...
    if (CP%DerivedParameters) then
        ...
        z_eq = (grhob+grhoc+grhogdm)/(grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates))) -1  !modified here by AD, grhogdm added in matter as it dominates there but gluon does not. This does not work if PT happens after lcdm z_eq.
        ...
        ThermoDerivedParams( derived_theta_rs_EQ ) = 100*rombint(dsound_da_exact,1d-8,a_eq,1d-6)/DA  !rombint error from here
        ...
        if (FeedbackLevel > 0) then
            ...
            write(*,'("tau_eq/Mpc           = ",f9.2)') timeOfz(z_eq)
            ...
            write(*,'("z_f                  = ",f8.2)') CP%zf
            write(*,'("tau_f/Mpc            = ",f9.6)') timeOfz(CP%zf)
            write(*,'("z_m                  = ",f8.2)') 1./(CP%af + (CP%af/CP%wf/2.)) - 1.
            write(*,'("z_p                  = ",f8.2)') 1./(CP%af - (CP%af/CP%wf/2.)) - 1.
            write(*,'("k_f Mpc              = ",f9.6)') 1./timeOfz(CP%zf)                                  !1./(CP%af*dtauda(CP%af))
            write(*,'("k_m Mpc              = ",f9.6)') 1./timeOfz(1./(CP%af + (CP%af/CP%wf/2.)) - 1.)     !1./( (CP%af + (CP%af/CP%wf/2.))*dtauda((CP%af + (CP%af/CP%wf/2.))))
            write(*,'("k_p Mpc              = ",f9.6)') 1./timeOfz(1./(CP%af - (CP%af/CP%wf/2.)) - 1.)     !1./( (CP%af - (CP%af/CP%wf/2.))*dtauda((CP%af - (CP%af/CP%wf/2.))))
