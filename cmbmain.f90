    ...
    fixq = CP%fixedqh*CP%h0/100. !modified here by AD
    if (fixq/=0._dl .and. CP%WantCls) CP%WantCls = .false.  !doing both fixq and scalar Cl gives error, hence...
    ...
    if (fixq==0._dl) EV%q=Evolve_q%points(q_ix)  !modified here by AD
    ...
    if (fixq==0._dl) EV%q_ix = q_ix  !modified here by AD
    ...
    if (fixq/=0._dl) call CalcScalarSources(EV,taustart)  !modified here by AD
    ...
    subroutine IonizationFunctionsAtTime(tau,tauminn,dlntau, opac, dopac, ddopac, &
    vis, dvis, ddvis, expmmu)
    real(dl), intent(in) :: tau,tauminn,dlntau
    real(dl), intent(out):: opac, dopac, ddopac, vis, dvis, ddvis, expmmu
    real(dl) d,dotmu(20000),ddotmu(20000),dddotmu(20000),ddddotmu(20000),emmu(20000),demmu(20000)
    integer i,nthermo
    ...
    *dlntau)
    ...
    implicit none
    type(EvolutionVars) EV
    real(dl) tau,tol1,tauend, taustart, tau_final,wdark,wdarkdot
    integer j,ind,itf,tau_points
    real(dl) c(24),w(EV%nvar,9), y(EV%nvar), sources(SourceNum)

    real(dl) yprime(EV%nvar),ddelta,delta,adotoa,dtauda,growth,redshift,mytanh
    real(dl) etak, hdot, phi_cn, psi_cn, camb_sigma, kappa,sigma
    real(dl) dgrho,grho,grhog_t,grhoc_t,grhob_t,grhor_t,grhogdm_t,grhonu_t,grhov_t
    real(dl) dgq,dgpi,pidot_sum,phi,phidot,gpres,gpres_nu,dgpi_diff,diff_rhopi
    real(dl) opacity, dopacity, ddopacity,visibility, dvisibility, ddvisibility
    real(dl) exptau, lenswindow,zeta
    external dtauda,mytanh
    ...
    ind=1
    tau_points=2500
    tau_final=TimeOfz(CP%fixedq_final_redshift)
    !if (fixq/=0._dl) then
     !  print*,'================================================================='
     !  print 105, CP%fixedqh, tau_final, CP%fixedq_final_redshift
!105    format (1x,("Computing transfer functions for k =",f10.5,1x, "h/Mpc with evolution upto tau =",f8.2,1x, "Mpc (z =",f8.1,")"))
     !  print*, ' '
    !endif
    !!Example code for plotting out variable evolution (can now do this better via python)
    if (fixq/=0._dl) then
        tol1=tol/exp(AccuracyBoost-1)
        call CreateTxtFile(CP%evolve_metric_file, 1)
        call CreateTxtFile(CP%evolve_perturb_file, 2)
!        call CreateTxtFile('evolve_metric.txt',1)
!        call CreateTxtFile('evolve_perturb.txt',2)
        do j=1,tau_points
            tauend = 10.**(log10(taustart) + (j-1)*(log10(tau_final)-log10(taustart))/tau_points)
            call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
            yprime = 0
            call derivs(EV,EV%ScalEqsToPropagate,tau,y,yprime)
            adotoa = 1/(y(1)*dtauda(y(1)))
            !ddelta= (yprime(EV%gdm_ix)*grhogdm+yprime(4)*grhob)/(grhob+grhogdm) !modified here, but not sure about correctness
            !delta=(grhogdm*y(EV%gdm_ix)+grhob*y(4))/(grhob+grhogdm)
            !growth= ddelta/delta/adotoa
            redshift=1._dl/y(1)-1.
            !hubble=adotoa/y(1)
            wdark = (1._dl - tanh((y(1)-CP%af)/(CP%af/CP%wf)))/6.
            !wdarkdot = -CP%wf/CP%af/6._dl*adotoa*y(1)*(cosh((y(1)-CP%af)/(CP%af/CP%wf)))**(-2)
            grhog_t=grhog/y(1)**2
            grhob_t=grhob/y(1)
            grhoc_t=grhoc/y(1)
            grhor_t=grhornomass/y(1)**2
            grhogdm_t=grhogdm*(CP%af/y(1))**(3._dl*wdark)/y(1)
            grhov_t=grhov*y(1)**2
            grho=grhonu_t+grhob_t+grhoc_t+grhogdm_t+grhor_t+grhog_t+grhov_t
            gpres_nu=0.
            if (EV%TightCoupling) then
                gpres=gpres_nu+ (grhog_t+grhor_t)/3 +grhogdm_t*wdark +grhov_t*(-1.)
            endif
            etak = -y(2)*2._dl
            !hdot = -yprime(3)/3.
            dgrho=grhob_t*y(4)+grhoc_t*y(3)+grhog_t*y(EV%g_ix)+grhor_t*y(EV%r_ix)+grhov_t*y(EV%w_ix) &
                    +grhogdm_t*y(EV%gdm_ix)*0.5_dl*(1._dl+tanh((y(1)-CP%af)/(CP%af/CP%wf))) &
                    +grhogdm_t*y(EV%gdm_ix)*0.5_dl*(1._dl-tanh((y(1)-CP%af)/(CP%af/CP%wf)))
            dgq=grhob_t*y(5) + grhogdm_t*y(EV%gdm_ix+1)*0.5_dl*(1._dl+tanh((y(1)-CP%af)/(CP%af/CP%wf)))&
                +grhog_t*y(EV%g_ix+1)+grhor_t*y(EV%r_ix+1)&
                +grhogdm_t*y(EV%gdm_ix+1)*0.5_dl*(1._dl-tanh((y(1)-CP%af)/(CP%af/CP%wf)))
            camb_sigma = (-yprime(3)/EV%q + 3._dl*yprime(2)/EV%q2)  !being computed from sigma = (z + 3/2*dgq/k2), see equations.f90
            !phi_cn = eta/2._dl + adotoa*camb_sigma/EV%q  !see section XV in CAMB notes
            !psi_cn = -kappa*y(1)*y(1)*(y(EV%g_ix+2) + y(EV%r_ix+2))/EV%q2 - phi_cn   !from astro-ph/0212248 eq. A3
            !psi_cn = 2.*phi - phi_cn
            dgpi  = grhor_t*y(EV%r_ix+2) + grhog_t*y(EV%g_ix+2)
            dgpi_diff = 0  !sum (3*p_nu -rho_nu)*pi_nu
            pidot_sum = grhog_t*yprime(EV%g_ix+2) + grhor_t*yprime(EV%r_ix+2)
            gpres=gpres_nu &
            +(grhog_t+grhor_t)/3+grhogdm_t*wdark +grhov_t*w_lam
            diff_rhopi = pidot_sum - (4*dgpi+ dgpi_diff)*adotoa
            sigma=(-yprime(3)/EV%q+1.5_dl*dgq/EV%q2)

            phi = -((dgrho +3*dgq*adotoa/EV%q)/EV%Kf(1) + dgpi)/(2*EV%q2)
            phidot = (1._dl/2._dl)*(adotoa*(-dgpi - 2*EV%q2*phi) + dgq*EV%q - &
                    diff_rhopi+ EV%q*sigma*(gpres + grho))/EV%q2

            zeta=((dgrho +3*dgq*adotoa/EV%q)/EV%Kf(1))/(2*EV%q2) &
                        +((y(EV%g_ix)+y(EV%r_ix))*3./4.+y(3)+y(4)+y(EV%gdm_ix)/(1.+wdark))/3._dl

            call IonizationFunctionsAtTime(tauend,taustart,(log10(tau_final)-log10(taustart))/tau_points,&
                    opacity, dopacity, ddopacity, &
                    visibility, dvisibility, ddvisibility, exptau)

            write (1,'(19E20.5E4)') tau,y(1),redshift,&
                                     grhog_t,grhoc_t,grhob_t,grhogdm_t,y(EV%g_ix),&
                                    dgrho,phi,phidot,dgq,dgpi,diff_rhopi,sigma,gpres,etak,exptau,zeta

            write (2,'(16E20.5E4)') tau, y(1), redshift, &
                                  y(EV%gdm_ix), y(EV%gdm_ix+1), &
                                  y(3), y(4), y(EV%g_ix), y(EV%g_ix+1),y(EV%g_ix+2), &
                                  y(EV%r_ix), y(EV%r_ix+1), y(EV%r_ix+2), &
                                  y(EV%gdm_ix)-3._dl*adotoa*(1.+wdark)*camb_sigma/EV%q,&  !perturbation in the Newtonian gauge, see section XV in CAMB notes
                                  y(EV%g_ix)-3._dl*adotoa*(4./3.)*camb_sigma/EV%q, &
                                  y(3)-3._dl*adotoa*camb_sigma/EV%q
                                  !-adotoa*(1.-3.*wdark)*y(EV%gdm_ix+1),wdarkdot/(1._dl+wdark)*y(EV%gdm_ix+1), &
                                  !wdark*EV%q*y(EV%gdm_ix)
        end do
        close(1)
        close(2)
        stop
...
101         if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
    ...
    do q_ix=Evolve_q%npoints+1,MT%num_q_trans
        EV%gdmflag=0  !modified here by AD
        ...
        EV%q2=EV%q**2
        EV%q_ix = q_ix
        tau = GetTauStart(EV%q)  !see above

        call GetNumEqns(EV)  !in equations.f90

        call GetTransfer(EV, tau)  !see below
    end do
    !$OMP END PARAllEl DO
