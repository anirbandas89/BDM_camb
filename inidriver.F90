    ...
    P%fixedqh  = Ini_Read_Double('fixed_kh_mode')
    if (P%fixedqh .ne. 0._dl) then
        P%fixedq_final_redshift = Ini_Read_Double('final_redshift')
        P%evolve_metric_file = Ini_Read_String('evolve_metric_filename')
        P%evolve_perturb_file = Ini_Read_String('evolve_perturbation_filename')
    endif

    P%gdmfrac = Ini_Read_Double('glueball_fraction')
    P%alpha0 = Ini_Read_Double('gluon_coupling_at_confinement')
    P%N_dark = Ini_Read_Int('dark_sector_N')

    if (Ini_Read_Logical('use_physical',.false.)) then   !modified here by AD
        P%omegab = Ini_Read_Double('ombh2')/(P%H0/100)**2
        P%omegan = Ini_Read_Double('omnuh2')/(P%H0/100)**2
        P%omegagdm = Ini_Read_Double('omgdmh2')*P%gdmfrac/(P%H0/100)**2
        P%omegac = ((1._dl-P%gdmfrac)*Ini_Read_Double('omgdmh2') + 1.e-7)/(P%H0/100)**2   !Ini_Read_Double('omch2')/(P%H0/100)**2
        P%omegav = 1- Ini_Read_Double('omk') - P%omegab-P%omegac - P%omegan-P%omegagdm
    else
        P%omegab = Ini_Read_Double('omega_baryon')
        P%omegav = Ini_Read_Double('omega_lambda')
        P%omegan = Ini_Read_Double('omega_neutrino')
        P%omegagdm = Ini_Read_Double('omega_glueball')*P%gdmfrac
        P%omegac = (1._dl-P%gdmfrac)*Ini_Read_Double('omega_glueball') + 1.e-7   !Ini_Read_Double('omega_cdm')
    end if
    
    P%zf  = Ini_Read_Double('gdm_redshift')
    P%af  = 1._dl/(1._dl+Ini_Read_Double('gdm_redshift'))
    P%wf  = Ini_Read_Double('transition_width')
    P%gluon_tca = Ini_Read_Logical('gluon_tca')
    P%cont_w = Ini_Read_Logical('continuous_EoS')

    !Gluon temperature at confinement using T_f^4 = (30/pi^2 g)*rho_gluon(a_f)
    P%Tf_gluon = sqrt(sqrt( (30._dl/((P%N_dark**2-1.)*const_pi**2))*(P%omegagdm / P%af**3 /0.7**2 * 2.1723e106_dl) )) !in Mpc^-1
