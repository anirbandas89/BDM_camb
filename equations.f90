    ...
    use ModelParams
    real(dl)  GetOmegak
    GetOmegak = 1 - (CP%omegab+CP%omegac+CP%omegav+CP%omegan+CP%omegagdm)   !modified here by AD
    ...
    a2=a**2
    !print*, 'grhogdm=',grhogdm, 'grhob',grhob/grhom
    !  8*pi*G*rho*a**4.
    grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass+ &
           grhogdm*0.5_dl*((a+CP%af) + tanh((a-CP%af)/(CP%af/CP%wf))*(a-CP%af))  !modified here by AD
    ...
    type EvolutionVars
        ...
        integer gdm_ix !Index of the gdm hierarchy        modified here by AD
        ...
        integer lmaxg,lmaxnr,lmaxnu,lmaxgpol,lmaxgdm,MaxlNeeded   !modified here by AD
        ...
        real(dl) pig, pigdot, pigdm, pigdmdot  !modified here by AD
        real(dl) poltruncfac
        ...
        integer gdmflag  !modified here by AD

    end type EvolutionVars

    ABSTRACT INTERFACE
    SUBROUTINE TSource_func(sources, tau, a, adotoa, grho, gpres,w_lam, cs2_lam,  &   !modified here by AD
        grhob_t,grhor_t,grhoc_t,grhog_t,grhov_t,grhonu_t,grhogdm_t, &
        k,etak, etakdot, phi, phidot, sigma, sigmadot, &
        dgrho, clxg,clxb,clxc,clxr, clxnu, clxde, clxgdm, delta_p_b, &
        dgq, qg, qr, qde, qgdm, vb, qgdot, qrdot, qgdmdot, vbdot, vgdmdot, &
        dgpi, pig, pir, pigdot, pirdot, pigdmdot, diff_rhopi, &
    ...
    if (next_switch < tauend) then
        ...
        if (next_switch == EV%TightSwitchoffTime) then
            ...
            !print*, 'vel-weighted nu hierarchy switch'
            do nu_i = 1, CP%Nu_mass_eigenstates
            ...
            !print*, 'late time nu hierarchy switch off'
            ind=1
            ...
            !print*, 'late time photon hierarchy switch off'
            ind=1
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!     modified here by AD
    ! GDM multipoles
    EV%gdm_ix= basic_num_eqns+1
    neq=neq + (EV%lmaxgdm+1)
    maxeq = maxeq +  (EV%lmaxgdm+1)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !!!!!! GDM maximum l, modified here by AD
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    lmax=EV%lmaxgdm
    yout(EVout%gdm_ix:EVout%gdm_ix+lmax)=y(EV%gdm_ix:EV%gdm_ix+lmax)
    ...
    EV%lmaxgdm = max(nint(3*lAccuracyBoost),3)     !modified here by AD
    ...
    if (CP%WantScalars) then
        ...
        !EV%lmaxgdm = max(nint(8*lAccuracyBoost),3)     !modified here by AD
        ...
        !EV%lmaxgdm = max(nint(14*lAccuracyBoost),3)     !modified here by AD
        ...
        if (CP%closed) then
            ...
            EV%lmaxgdm=min(EV%lmaxgdm, EV%FirstZerolForBeta-1)   !modified here by AD
        end if
        !if (CP%gluon_tca) EV%lmaxgdm = 1

        EV%poltruncfac=real(EV%lmaxgpol,dl)/max(1,(EV%lmaxgpol-2))
        EV%MaxlNeeded=max(EV%lmaxg,EV%lmaxnr,EV%lmaxgpol,EV%lmaxnu,EV%lmaxgdm)  !modified here by AD
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        i_qg=5,i_qr=6,i_vb=7,i_pir=8, i_eta=9, i_aj3r=10,i_clxde=11,i_vde=12,i_clxgdm=13, &
        i_qgdm=14, i_pigdm=15, i_aj3gdm=16                     !modified here by AD
    integer, parameter :: i_max = i_aj3gdm
    ...
    om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhogdm*CP%af))   !modified here by AD. The gdm density is added in the radiation part
    omtau=om*tau                                        !since the initial time is in the radiation dominated era. Inspired by AxionCAMB
    Rv=grhonu/(grhonu+grhog+grhogdm*CP%af)
    ...
    initv(1,i_clxgdm)=initv(1,i_clxg)      !modified here by AD
    initv(1,i_qgdm)=initv(1,i_qg)
    initv(1,i_pigdm)=initv(1,i_pir)*0.   !using massless neutrino anisotropic stress and higher multipoles, but that might not be correct!!!! Maybe define Rgdm...
    initv(1,i_aj3gdm)=initv(1,i_aj3r)*0.
    
                                                                !!!!!!!!!!WARNING!!!!!!!!!!
    if (CP%Scalar_initial_condition/= initial_adiabatic) then   !gdm initial condition is not added, for the time being, to the other initial conditions sections.
        !CDM isocurvature
    ...
    ! GDM     modified here by AD
    y(EV%gdm_ix)=InitVec(i_clxgdm)
    y(EV%gdm_ix+1)=InitVec(i_qgdm)
    y(EV%gdm_ix+2)=InitVec(i_pigdm)*.0
    if (EV%lmaxgdm>2) then
        y(EV%gdm_ix+3)=InitVec(i_aj3gdm)*0.
    endif
    ...
    !  gdm equations are not added in the tensor or vector sections
    use ThermoData
    implicit none
    ...
    !write out clxc, clxb, clxg, clxn, clxgdm
    implicit none
    type(EvolutionVars) EV
    ...
    use ThermoData
    use MassiveNu
    use cnumbers    !modified here by AD
    implicit none
    type(EvolutionVars) EV

    integer n,nu_i
    real(dl) ay(n),ayprime(n)
    real(dl) tau,w,dtauda,redshift
    real(dl) k,k2
    ...
    real(dl) photbar,cs2,pb43,grho,slip,clxgdot, &
        clxcdot,clxbdot,adotdota,gpres,clxrdot,clxgdmdot,etak    !modified here by AD
    real(dl) q,aq,v
    real(dl) G11_t,G30_t, wnu_arr(max_nu)

    real(dl) dgq,grhob_t,grhor_t,grhoc_t,grhog_t,grhov_t,grhonu_t, grhogdm_t,sigma,polter
    real(dl) qgdot,qrdot,qgdmdot,pigdot,pirdot,pigdmdot,vbdot,vgdmdot,dgrho,adotoa
    real(dl) a,a2,z,clxc,clxb,vb, clxg,qg,pig,clxr,qr,pir
    real(dl) clxgdm,qgdm,pigdm,wdark,wdarkdot
    ...
    real(dl) n_gluon, alpha_dark, sigma_gg, gamma_gg  ! g-g interaction variables
    !non-flat vars
    ...
    ! GDM         modified here by AD
    wdark = (1._dl - tanh((a-CP%af)/(CP%af/CP%wf)))/6._dl
    grhogdm_t = grhogdm*(CP%af/a)**(3._dl*wdark)/a
    !if(a .ge. CP%af) then
     !   grhogdm_t=grhogdm/a
    !else
     !   grhogdm_t=grhogdm*CP%af/a2
    !endif
    ...
    !  8*pi*a*a*SUM[rho_i*clx_i]      modified here by AD
    dgrho_matter=grhob_t*clxb+grhoc_t*clxc+ grhogdm_t*clxgdm*0.5_dl*(1._dl+tanh((a-CP%af)/(CP%af/CP%wf)))
    
    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]     modified here by AD
    dgq=grhob_t*vb + grhogdm_t*qgdm*0.5_dl*(1._dl+tanh((a-CP%af)/(CP%af/CP%wf)))
    ...
    !modified here by AD
    grho_matter=grhonu_t+grhob_t+grhoc_t+grhogdm_t*0.5_dl*(1._dl+tanh((a-CP%af)/(CP%af/CP%wf)))
    grho = grho_matter+grhor_t+grhog_t+grhov_t+grhogdm_t*0.5_dl*(1._dl-tanh((a-CP%af)/(CP%af/CP%wf)))

    if (CP%flat) then
        adotoa=1._dl/(a*dtauda(a))             !sqrt(grho/3)
        cothxor=1._dl/tau
    else
    ...
    clxgdm=ay(EV%gdm_ix)
    qgdm  =ay(EV%gdm_ix+1)
    pigdm =ay(EV%gdm_ix+2)*0.

    !  8*pi*a*a*SUM[rho_i*clx_i] - radiation terms
    dgrho=dgrho + grhog_t*clxg+grhor_t*clxr+grhogdm_t*clxgdm*0.5_dl*(1._dl-tanh((a-CP%af)/(CP%af/CP%wf)))  !modified here by AD

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    !modified here by AD
    dgq=dgq + grhog_t*qg+grhor_t*qr+grhogdm_t*qgdm*0.5_dl*(1._dl-tanh((a-CP%af)/(CP%af/CP%wf)))
    ...
    if (EV%TightCoupling) then
        !  ddota/a
        gpres=gpres_nu+ (grhog_t+grhor_t+grhogdm_t*0.5_dl*(1._dl-tanh((a-CP%af)/(CP%af/CP%wf))))/3 +grhov_t*w_lam
    ...
    wdark = (1._dl - tanh((a-CP%af)/(CP%af/CP%wf)))/6.                            !continuous EoS of gluon-glueball species
    !wdarkdot = -CP%wf/CP%af/6._dl*adotoa*a*(cosh((a-CP%af)/(CP%af/CP%wf)))**(-2)  !d(wdark)/dtau

    if (CP%cont_w) then

        clxgdmdot = -k*(qgdm + (1._dl + wdark)*z)
        qgdmdot = -adotoa*(1._dl - 3.*wdark)*qgdm + wdark*k*clxgdm        !+ wdarkdot/(1._dl+wdark)*qgdm
        ayprime(EV%gdm_ix)=clxgdmdot
        ayprime(EV%gdm_ix+1)=qgdmdot
        ayprime(EV%gdm_ix+2)=0.
        if (EV%lmaxgdm>2) then
           do l=3,EV%lmaxgdm-1
               ayprime(EV%gdm_ix+l)=0.
           enddo
        endif
        ...
        if (a .ge. CP%af) then
            ...
            if (EV%gdmflag .eq. 0) then    !After transition vgdm will start with (3/4)*qgdm. Later on qgdm is used as vgdm. No separate variable declared for vgdm.
                clxgdmdot=-k*(z+0.75_dl*qgdm)
                ayprime(EV%gdm_ix)=clxgdmdot
                vgdmdot=-adotoa*0.75_dl*qgdm
                ayprime(EV%gdm_ix+1)=vgdmdot
                EV%gdmflag=1
            else
                clxgdmdot=-k*(z+qgdm)
                vgdmdot=-adotoa*qgdm
            ...
            !n_gluon = (zeta3*(CP%N_dark**2-1._dl)/const_pi**2) * CP%Tf_gluon**3 * (CP%af/a)**3
            !alpha_dark = CP%alpha0/(1._dl+(11.*CP%alpha0*log(CP%af**2/a**2)/(4.*const_pi)))
            !sigma_gg = CP%N_dark**2*(CP%N_dark**2 - 1._dl)*alpha_dark**2 * (a/CP%af)**2 /CP%Tf_gluon**2  !last factor is the conversion from GeV^-2 to Mpc^2
            !gamma_gg = n_gluon*sigma_gg
    
            clxgdmdot=-k*(4._dl/3._dl*z+qgdm)
            ayprime(EV%gdm_ix)=clxgdmdot
    
            if (.not.(CP%gluon_tca)) then !checking for gluon TCA
              qgdmdot=EV%denlk(1)*clxgdm-EV%denlk2(1)*pigdm
              ayprime(EV%gdm_ix+1)=qgdmdot
              !higher multipoles in gluon hierarchy        
              ix=EV%gdm_ix+2
                if (EV%lmaxgdm>2) then
                    pigdmdot=EV%denlk(2)*qgdm- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma !- pigdm*(4._dl/3./k)*gamma_gg
                    ayprime(ix)=pigdmdot
                    do l=3,EV%lmaxgdm-1
                        ix=ix+1
                        ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1)) !- ay(ix)*gamma_gg
                    end do
                    !  Truncate the gluon expansion
                    ix=ix+1
                    ayprime(ix)=k*ay(ix-1)- (EV%lmaxgdm+1)*cothxor*ay(ix)
                else
                    pigdmdot=EV%denlk(2)*qgdm +8._dl/15._dl*k*sigma !- pigdm*(4._dl/3./k)*gamma_gg
                    ayprime(ix)=pigdmdot
                endif
            else
              qgdmdot=EV%denlk(1)*clxgdm  !for gluon TCA, pigdm is zero
              ayprime(EV%gdm_ix+1)=qgdmdot
              !higher multipoles in gluon hierarchy        
              ix=EV%gdm_ix+2
                if (EV%lmaxgdm>2) then
                    pigdmdot=EV%denlk(2)*qgdm- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma
                    ayprime(ix)=pigdmdot*0
                    do l=3,EV%lmaxgdm-1
                        ix=ix+1
                        ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1))*0
                    end do
                    !  Truncate the gluon expansion
                    ix=ix+1
                    ayprime(ix)=k*ay(ix-1)- (EV%lmaxgdm+1)*cothxor*ay(ix)
                else
                    pigdmdot=EV%denlk(2)*qgdm +8._dl/15._dl*k*sigma
                    ayprime(ix)=pigdmdot*0
                endif
            endif
        endif
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !modified here by AD
    !    if (CP%fixedqh/=0._dl) then
    !        redshift=1._dl/a-1._dl
    !        dgpi  = grhor_t*pir + grhog_t*pig + grhogdm_t*pigdm*0
    !        dgpi_diff = 0  !sum (3*p_nu -rho_nu)*pi_nu
    !        pidot_sum = grhog_t*pigdot + grhor_t*pirdot &
    !                    + 0*grhogdm_t*pigdmdot*0.5*(1._dl - tanh((a-CP%af)/(CP%af/CP%wf)))
    !        gpres=gpres_nu &
    !            +(grhog_t+grhor_t+grhogdm_t*0.5*(1._dl - tanh((a-CP%af)/(CP%af/CP%wf))))/3 +grhov_t*w_lam
    !        diff_rhopi = pidot_sum - (4*dgpi+ dgpi_diff)*adotoa

    !        phi = -((dgrho +3*dgq*adotoa/k)/EV%Kf(1) + dgpi)/(2*k2)
    !        phidot = (1._dl/2._dl)*(adotoa*(-dgpi - 2*k2*phi) + dgq*k - &
    !                    diff_rhopi+ k*sigma*(gpres + grho))/k2
    !        call IonizationFunctionsAtTime(tau, opacity, dopacity, ddopacity, &
    !                    visibility, dvisibility, ddvisibility, exptau, lenswindow)

    !        !print*,redshift,phi,phidot,adotoa*(-dgpi - 2*k2*phi)/k2,k*sigma,(gpres + grho)/k2,dgq*k/k2, &
    !        !        diff_rhopi/k2,EV%Kf(1)
    !        print*, redshift,qg,qr,pig,pir,pigdot,pirdot,sigma,clxg,clxr,clxgdm,dgrho,etak,dgq
    !    endif
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        !if (a .ge. CP%af) then     !modified here by AD, no tanh() smoothing here as dgpi goes to derivatives only
        !    dgpi  = grhor_t*pir + grhog_t*pig
        !    dgpi_diff = 0  !sum (3*p_nu -rho_nu)*pi_nu
        !    pidot_sum = grhog_t*pigdot + grhor_t*pirdot
        !else
        !    dgpi  = grhor_t*pir + grhog_t*pig + grhogdm_t*pigdm
        !    dgpi_diff = 0  !sum (3*p_nu -rho_nu)*pi_nu
        !    pidot_sum = grhog_t*pigdot + grhor_t*pirdot + grhogdm_t*pigdmdot
        !endif
        ...
        diff_rhopi = pidot_sum - (4*dgpi+ dgpi_diff)*adotoa
        gpres=gpres_nu &
             +(grhog_t+grhor_t)/3.+grhogdm_t*wdark+grhov_t*w_lam

        !if (a .ge. CP%af) then         !modified here by AD, no tanh() smoothing here as gpres goes to derivatives only
        !    gpres=gpres_nu+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
        !else
        !    gpres=gpres_nu+ (grhog_t+grhor_t+grhogdm_t)/3 +grhov_t*w_lam
        !endif
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            EV%OutputTransfer(Transfer_gdm) = clxgdm    !modified here by AD
            EV%OutputTransfer(Transfer_tot) =  dgrho_matter/grho_matter !includes neutrinos
            EV%OutputTransfer(Transfer_nonu) = (grhob_t*clxb+grhoc_t*clxc+grhogdm_t*clxgdm) &
                                                /(grhob_t + grhoc_t + grhogdm_t)   !modified here by AD
            ...
            EV%OutputTransfer(Transfer_vel_baryon_cdm) = vb  !Not the true cdm-baryon relative velocity anymore
        end if
        if (associated(EV%OutputSources)) then
            ...
            monopole_source =  (-etak/(k*EV%Kf(1)) + 2._dl*phi + clxg/4._dl)*visibility
            ...
            if (associated(EV%CustomSources)) then                                                               
                call custom_sources_func(EV%CustomSources, tau, a, adotoa, grho, gpres,w_lam, cs2_lam, &         
                    ...
                    dgrho, clxg,clxb,clxc,clxr, clxnu, clxde, clxgdm, cs2*clxb, &
                    dgq, qg, qr, qde, qgdm, vb, qgdot, qrdot,qgdmdot, vbdot,vgdmdot, &
                    dgpi, pig, pir, pigdot, pirdot,pigdmdot, diff_rhopi, &
                    polter, polterdot, polterddot, octg, octgdot, E, Edot, &
                    opacity, dopacity, ddopacity, visibility, dvisibility, ddvisibility, exptau, &
    ...
    real(dl) sigma, qg,pig, qr, vb, rhoq, vbdot, vgdmdot, photbar, pb43
